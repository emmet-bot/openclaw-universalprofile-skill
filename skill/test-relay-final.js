...
// Simple relay test with setData
console.log('\n📝 Testing Relay API...');
const testKey = ethers.keccak256(ethers.toUtf8Bytes('RelayTest-' + Date.now()));
const testValue = ethers.toUtf8Bytes('SUCCESS');
...
console.log('✍️  Signature verified locally:', ethers.recoverAddress(hash, signature) === CONTROLLER_ADDRESS ? '✅' : '❌');
...
if (response.ok && result.transactionHash) {
  console.log('\n🎉 SUCCESS! Relayer API works with SIGN permission!');
